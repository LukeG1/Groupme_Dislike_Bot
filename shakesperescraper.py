...
phrase = ["artless","bawdy","beslubbering","bootless","churlish","clouted",
...
def getInsult(name):
    full = name.strip()
    full += "! Thou "
    full += phrase[random.randrange(len(phrase)-1)] + " "
    full += second_phrase[random.randrange(len(second_phrase)-1)] + " "
    full += third_phrase[random.randrange(len(third_phrase)-1)]
    return full

#print(getInsult("Luke"))